/**
 * HTML sanitizer used by the reader (article extraction) and mail (HTML
 * body rendering). Whitelists a small set of structural tags and rewrites
 * URLs against `baseUrl`. Server-only — pulls in linkedom — so call it
 * from server functions, never from the browser bundle.
 */
