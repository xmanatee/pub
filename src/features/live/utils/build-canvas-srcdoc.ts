...
function buildCanvasBridgeScript(bridgeToken: string): string {
  ...
  return [
    ...
    'function toErrorMessage(error){if(!error){return"Unknown error";}if(error&&error.message){return String(error.message);}return String(error);}',
    'function notify(payload){if(notifyFailed){return;}payload=payload||{};payload.source="pubblue-canvas";payload.bridgeToken=bridgeToken;try{parent.postMessage(payload,"*");}catch(error){notifyFailed=true;if(typeof console!=="undefined"&&typeof console.warn==="function"){console.warn("pubblue canvas bridge postMessage failed",error);}}}',
    ...
    "function scheduleManifestBindRetry(){if(!manifestPayload||manifestBound){return;}if(manifestRetryCount>=20){return;}manifestRetryTimer=setTimeout(function(){manifestRetryTimer=null;emitManifestBind();scheduleManifestBindRetry();},400);}",
...
export function buildCanvasSrcDoc(html: string, options?: BuildCanvasSrcDocOptions): string {
  const script = buildCanvasBridgeScript(options?.bridgeToken ?? "pubblue-bridge");
