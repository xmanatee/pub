function buildCanvasBridgeScript(): string {
  return [
    ...
    'function notify(payload){if(notifyFailed){return;}payload=payload||{};payload.source="pubblue-canvas";try{parent.postMessage(payload,"*");}catch(error){notifyFailed=true;if(typeof console!=="undefined"&&typeof console.warn==="function"){console.warn("pubblue canvas bridge postMessage failed",error);}}}',
