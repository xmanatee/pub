...
- Are all specified features implemented? List any that are missing.
- Do colors, fonts, and spacing match the spec? Check CSS custom properties against the palette in the design doc.
- Are all interactions described in the spec present and working as described?
- Are all states (empty, active, loaded, error) handled as specified?
- If the implementation intentionally deviates from the spec, is the deviation an improvement or an oversight?
...
### 5. Command System (if tool-powered)
...
<instructions>
- Fix every issue you find by writing the corrected `index.html` (and `meta.json` if needed) using the Write tool.
- If you find no issues at all, do not modify the files — just report that everything looks good.
- Do NOT simplify, remove features, or refactor for its own sake — only fix real bugs, add missing edge case handling, and close gaps vs the design spec.
- Do NOT add comments, docstrings, or attribution to the code.
- After reviewing and fixing, briefly list what you changed and why.
</instructions>
