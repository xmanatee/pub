...
export function PubCard({ pub, viewCount, isLive, onToggleVisibility, onDelete }: PubCardProps) {
  const hasContent = pub.content || (pub.contentSize ?? 0) > 0;
  return (
      ...
      <Link to="/p/$slug" params={{ slug: pub.slug }} className="block">
        <div className="aspect-[1200/630] overflow-hidden bg-white relative">
          {!hasContent ? (
            ...
            <PubPreviewIframe
              content={pub.content}
              htmlSrc={!pub.content ? `/serve/${pub.slug}?preview=1` : undefined}
        ...
        <div className="text-xs text-muted-foreground">
          ...
          {hasContent && (
            <span className="tabular-nums">
              {" "}
              &middot; {(() => {
                const size = pub.content ? new Blob([pub.content]).size : (pub.contentSize ?? 0);
                const kb = size / 1024;
